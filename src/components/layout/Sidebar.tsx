...
export const Sidebar: React.FC<SidebarProps> = ({ isCollapsed = false }) => {
    ...
    return (
        <div className={`fixed left-0 top-0 h-full bg-white border-r border-gray-200 transition-all duration-200 z-40 ${isCollapsed ? 'w-16' : 'w-64'
            }`}>
                        ...
                        {navigation.map((item) => {
                            ...
                            return (
                                <li key={item.name}>
                                    <Link
                                        href={item.href}
                                        className={`flex items-center px-3 py-3 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors group ${isActive ? 'bg-gray-100 font-semibold' : ''
                                            }`}
                                    >
                                        <Icon className={`w-6 h-6 ${isActive ? 'text-black' : 'text-gray-700'} group-hover:text-black`} />
                                        {!isCollapsed && (
                                            <span className={`ml-3 text-base ${isActive ? 'font-semibold text-black' : 'text-gray-700'} group-hover:text-black`}>
                                                {item.name}
                                            </span>
                                        )}
                                    </Link>
